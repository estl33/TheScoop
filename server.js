...
function getOrCreateComment(url, request) {
  ...
  if (reqComment && reqComment.body && database.users[reqComment.username] && database.articles[reqComment.articleId]) {
    const comment = {
...
function updateComment(url, request) {
  ...
  if (!id || !reqComment || !reqComment.body) {
    response.status = 400;
  } else if (!savedComment) {
    response.status = 404;
  } else {
    database.comments[id].body = reqComment.body;
